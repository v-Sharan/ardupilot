...
#define STRIKE_MIN_GS_MS          5.0f
#define STRIKE_MAX_PITCH_DOWN_CD  8800     // 88 deg
#define STRIKE_MAX_ROLL_CD        8500
#define STRIKE_WIND_COMP_LIMIT    0.35f
...
{
    const int32_t lat = plane.aparm.str_lat.get();
    const int32_t lon = plane.aparm.str_lon.get();
...
void ModeStrike::update()
{
    ...
    // ---------- CURRENT STATE ----------
    const Location &curr = plane.current_loc;
    const float alt_agl = MAX(plane.relative_altitude, 0.1f);

    // ---------- DISTANCE ----------
    const float dx =
        (target_location.lat - curr.lat) * 1e-7f * METERS_PER_DEG_LAT;

    const float lat_rad = radians(curr.lat * 1e-7f);
    const float dy =
        (target_location.lng - curr.lng) * 1e-7f *
        METERS_PER_DEG_LAT * cosf(lat_rad);

    const float horizontal_distance = sqrtf(dx * dx + dy * dy);

    // ---------- TERMINATION ----------
    const int16_t min_dist = plane.aparm.str_min_dis.get();
    if (horizontal_distance < min_dist || alt_agl < STRIKE_MIN_ALT_AGL_M) {
        strike_complete = true;
        gcs().send_text(MAV_SEVERITY_CRITICAL, "STRIKE: IMPACT");
        return;
    }

    // ---------- BEARING ----------
    float target_bearing = atan2f(dy, dx);

    // ---------- WIND COMP ----------
    Vector3f wind;
    if (ahrs.wind_estimate(wind)) {
        const float wind_dir = atan2f(-wind.y, -wind.x);
        const float wind_cross = wind.length() * sinf(wrap_PI(wind_dir - target_bearing));

        const float gs = MAX(ahrs.groundspeed(), STRIKE_MIN_GS_MS);
        const float drift = wind_cross * (horizontal_distance / gs);

        const float corr = atan2f(drift, horizontal_distance);
        target_bearing -= constrain_float(corr, -STRIKE_WIND_COMP_LIMIT, STRIKE_WIND_COMP_LIMIT);
    }

    // ---------- AGGRESSION SCALE ----------
    const int16_t term_dist = plane.aparm.str_term_dis.get();
    float aggression = 1.0f - (horizontal_distance / term_dist);
    aggression = constrain_float(aggression, 0.0f, 1.0f);

    // ---------- FLIGHT PATH ANGLE ----------
    const float gamma_rad = atan2f(alt_agl, horizontal_distance);
    float pitch_cmd_deg = degrees(gamma_rad) + 2.0f;   // AoA bias
    pitch_cmd_deg *= (0.4f + 0.6f * aggression);

    pitch_cmd_deg = constrain_float(pitch_cmd_deg, 5.0f, 85.0f);
    plane.nav_pitch_cd = -pitch_cmd_deg * 100.0f;

    // ---------- ROLL CONTROL ----------
    plane.calc_nav_roll();

    const float bearing_error =
        wrap_PI(target_bearing - ahrs.yaw_sensor * 0.01f);

    plane.nav_roll_cd += bearing_error * aggression * 6000.0f;
    plane.nav_roll_cd =
        constrain_int32(plane.nav_roll_cd, -STRIKE_MAX_ROLL_CD, STRIKE_MAX_ROLL_CD);
    ...
    if (aggression < 0.3f) {
        ...
        throttle = plane.aparm.throttle_max.get() * 0.9f;
    } else {
        throttle = plane.aparm.throttle_max.get();
    }

    if (horizontal_distance < 10.0f) {
        throttle = 0;  // ballistic terminal
    }

    SRV_Channels::set_output_scaled(SRV_Channel::k_throttle, throttle);

    // ---------- NAV UPDATE ----------
    plane.prev_WP_loc = curr;
    plane.next_WP_loc = target_location;
    plane.nav_controller->update_waypoint(curr, target_location);
}
...
{
    plane.set_guided_WP(target_location);
    plane.update_loiter(5.0f);   // force straight-in
}

// ===================================================

bool ModeStrike::handle_guided_request(Location target)
{
    plane.fix_terrain_WP(target, __LINE__);
